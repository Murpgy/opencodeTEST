...
describe("session.compaction.upstream toggle", () => {
  // Effect-native env scoping: set the toggle for the duration of the wrapped
  // effect, restore afterwards on all exits (ensuring covers interrupts, so
  // concurrent legacy tests elsewhere in the file cannot observe the toggle).
  const withUpstream = <A, E, R>(effect: Effect.Effect<A, E, R>, value: string | undefined) =>
    Effect.gen(function* () {
  ...
  itCompaction.instance(
    "upstream sends one text message with the tagged conversation",
    () => {
      const stub = llm()
      ...
      return Effect.gen(function* () {
        ...
        expect(captured.indexOf("[User]: older context")).toBeLessThan(captured.indexOf("Create a new anchored summary"))
  ...
  itCompaction.instance(
    "upstream anchors repeats with prior-summary tags",
    () => {
      const stub = llm()
      let captured = ""
      stub.push(reply("summary one"))
  ...
  itCompaction.instance(
    "upstream keeps plugin context outside the serialized conversation",
    () => {
      const stub = llm()
      let captured = ""
      stub.push(
      ...
      return Effect.gen(function* () {
        ...
        expect(parent).toBeTruthy()
        yield* withUpstream(
          SessionCompaction.use.process({ parentID: parent!, messages: msgs, sessionID: session.id, auto: false }).pipe(
            withCompaction({
              llm: stub.llmLayer,
        ...
        expect(captured.indexOf("</conversation>")).toBeLessThan(captured.indexOf("Prioritize unresolved migration details"))
      })
    },
    { git: true },
  )
  ...
  itCompaction.instance(
    "upstream serializes repeated compaction history as one user message",
    () => {
      const stub = llm()
